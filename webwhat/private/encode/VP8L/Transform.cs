namespace VP8L {
  ...
  internal static class Transform {
    ...
    internal static Image PaletteTransform(BitWriter b, Image image, Palette palette) {
	  ...
	  Console.WriteLine("palette transform");
    ...
    static void WritePalette(BitWriter b, Palette palette) {
	  ...
	  var black = new Argb(255, 0, 0, 0);
	  var hide = new Argb(255, 255, 0, 255);
      for(int i = 0; i < palette.Count; ++i) {
		  if (i == 0) {
			  if (palette.Colors[0] != hide) {
				  image.Pixels[i] = palette.Colors[0];
			  } else {
				  image.Pixels[i] = black;
			  }
		  } else {
			  if (palette.Colors[i] != hide) {
				  image.Pixels[i] = palette.Colors[i] - palette.Colors[i-1];
			  } else {
				  image.Pixels[i] = black - palette.Colors[i-1];
			  }
